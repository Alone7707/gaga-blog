    ...
    <div class="grid gap-6 xl:grid-cols-[minmax(0,1.2fr)_360px] xl:items-start">
      <div>
        ...
        <div v-if="meta?.length" class="mt-6 flex flex-wrap gap-3 text-sm text-[var(--text-3)]">
          <span v-for="item in meta" :key="item" class="ui-badge">{{ item }}</span>
        </div>

        <div v-if="actions?.length" class="mt-6 flex flex-wrap gap-3">
          <RouterLink
      ...
      <div v-if="asideTitle || asideText || asideStats?.length" class="rounded-[24px] border border-[var(--line-soft)] bg-[var(--bg-card-soft)] p-5">
        <p class="editor-kicker">页面概览</p>
        <h3 v-if="asideTitle" class="mt-3 text-[20px] text-[var(--text-1)] font-semibold">
          {{ asideTitle }}
        </h3>
